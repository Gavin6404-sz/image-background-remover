...
export default function Home() {
  ...
  const handleEmailLogin = async (username: string, password: string) => {
    if (!username || !password) {
      toast.error(lang === 'en' ? 'Please enter username and password' : '请输入用户名和密码');
      return;
    }
    ...
    try {
      ...
      const data: { success?: boolean; user?: any; sessionToken?: string; error?: string } = await res.json();
  ...
  const handleRegister = async (username: string, password: string, confirmPassword: string) => {
    if (password !== confirmPassword) {
      toast.error(lang === 'en' ? 'Passwords do not match' : '两次密码输入不一致');
      return;
    }

    if (username.length < 3 || username.length > 20) {
      toast.error(lang === 'en' ? 'Username must be 3-20 characters' : '用户名需要 3-20 个字符');
      return;
    }

    if (!/^[a-zA-Z0-9_]+$/.test(username)) {
      toast.error(lang === 'en' ? 'Username can only contain letters, numbers, and underscores' : '用户名只能包含字母、数字和下划线');
      return;
    }

    if (password.length < 8) {
      toast.error(lang === 'en' ? 'Password must be at least 8 characters' : '密码至少需要 8 个字符');
      return;
    }
    ...
    try {
      ...
      toast.error(data.error || (lang === 'en' ? 'Registration failed' : '注册失败'));
    } catch {
      toast.error(lang === 'en' ? 'Registration failed' : '注册失败');
    } finally {
      setIsLoggingIn(false);
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              ...
              <>
                ...
                <Dialog open={showLoginDialog} onOpenChange={(open) => { setShowLoginDialog(open); if (!open) setIsRegistering(false); }}>
                  <DialogContent className="sm:max-w-[420px] p-0 gap-0 rounded-2xl shadow-2xl border border-gray-200 overflow-hidden bg-white">
                    {/* Brand header */}
                    <div className="w-full bg-gradient-to-r from-orange-500 via-amber-400 to-orange-400 px-8 py-7">
                      {/* Logo mark */}
                      <div className="w-12 h-12 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center mb-4 shadow-lg">
                      ...
                      <h2 className="text-2xl font-black text-white leading-tight">
                        {isRegistering
                          ? (lang === 'en' ? 'Create your account' : '创建账号')
                          : (lang === 'en' ? 'Welcome back' : '欢迎回来')}
                      </h2>
                      <p className="text-orange-100 text-sm mt-1">
                        {isRegistering
                          ? (lang === 'en' ? 'Start removing backgrounds for free' : '免费开始去除背景')
                          : (lang === 'en' ? 'Sign in to continue to Image Toolbox' : '登录以继续使用')}
                      </p>
                    </div>
                    ...
                    <div className="px-8 py-7">
                      ...
                      <div className="flex flex-col gap-2.5 mb-5">
                        ...
                        <button
                          ...
                          className="w-full h-12 flex items-center justify-center gap-3 rounded-xl bg-gradient-to-r from-blue-200 to-blue-100 border border-blue-400 hover:border-blue-500 hover:shadow-lg hover:shadow-blue-300/60 text-sm font-bold text-blue-700 transition-all duration-200 relative overflow-hidden group"
                        >
                          ...
                          <span>{lang === 'en' ? 'Continue with Google' : '使用 Google 账号'}</span>
                        </button>

                        {/* GitHub */}
                        <a
                          href={`${API_BASE_URL}/api/auth/github`}
                          className="w-full h-12 flex items-center justify-center gap-3 rounded-xl bg-gradient-to-r from-blue-200 to-blue-100 hover:from-blue-300 hover:to-blue-200 border border-blue-400 hover:border-blue-500 hover:shadow-lg hover:shadow-blue-300/60 text-sm font-bold text-blue-700 transition-all duration-200 no-underline"
                        >
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="#1f2328" className="flex-shrink-0">
                          ...
                          <span>{lang === 'en' ? 'Continue with GitHub' : '使用 GitHub 账号'}</span>
                        </a>
                      </div>
                      ...
                      <div className="flex flex-col gap-4">
                        ...
                        <div className="space-y-1.5">
                          <label className="text-sm font-bold text-orange-500 tracking-wide">
                            {lang === 'en' ? 'Username' : '用户名'}
                          </label>
                          <input
                            id="username-input"
                            type="text"
                            placeholder={lang === 'en' ? 'Choose a username' : '设置用户名'}
                            className="w-full h-12 px-4 rounded-xl border-2 border-orange-300 bg-orange-50/50 text-gray-900 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-orange-400 focus:bg-white transition-all duration-150"
                            autoComplete="username"
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') {
                                if (isRegistering) {
                                  (document.getElementById('confirm-password-input') as HTMLInputElement)?.focus();
                                } else {
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input
                            id="password-input"
                            type="password"
                            placeholder={lang === 'en' ? 'Enter your password' : '输入密码'}
                            className="w-full h-12 px-4 rounded-xl border-2 border-orange-300 bg-orange-50/50 text-gray-900 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-orange-400 focus:bg-white transition-all duration-150"
                        ...
                        {isRegistering && (
                          <div className="space-y-1.5">
                            ...
                            <input
                              ...
                              type="password"
                              placeholder={lang === 'en' ? 'Confirm your password' : '再次输入密码'}
                              className="w-full h-12 px-4 rounded-xl border-2 border-orange-300 bg-orange-50/50 text-gray-900 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-orange-400 focus:bg-white transition-all duration-150"
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                  ...
                                  if (usernameEl && passwordEl && confirmEl) {
                                    handleRegister(usernameEl.value, passwordEl.value, confirmEl.value);
                                  }
                                }
                              }}
                            />
                        ...
                        {/* Submit Button */}
                        <Button
                          className="w-full h-12 rounded-xl font-bold text-base bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white border-0 shadow-lg shadow-orange-500/30 transition-all duration-150 mt-1"
                          onClick={() => {
                            const usernameEl = document.getElementById('username-input') as HTMLInputElement;
                            const passwordEl = document.getElementById('password-input') as HTMLInputElement;
                            const confirmEl = document.getElementById('confirm-password-input') as HTMLInputElement;
                            ...
                            if (isRegistering) {
                              ...
                              handleEmailLogin(usernameEl.value, passwordEl.value);
                            }
                          }}
                        ...
                        {/* Toggle */}
                        <p className="text-center text-sm text-gray-500 mt-0.5">
                          {isRegistering ? (
                            <>
                              ...
                              <button
                                ...
                                className="text-orange-500 hover:text-orange-600 font-bold transition-colors cursor-pointer bg-transparent border-0"
                              >
